...
#' \itemize{
#'   \item ENSEMBL - A character vector of ensembl identifiers for genes needed in the model.
...
#'   \item betaCoef - A numeric vector of beta coefficients for genes that are weighted towards IdentifiHR predictions. NAs indiciate that a gene is only used in normalisation.
#'   \item inputStatus - A character vector indiciating if a gene was "present" in the input data. NAs indiciate that a gene was missing.
#'   \item normWeightedGene - A character vector indicating if a gene contributes to only normalisation, being "normGene", or contributes to both normalisation and model predictions, bbeing "normWeightedGene".
#' }
...
interrogateMissingness <- function(y,
  ...
  if (geneIds == "ENSEMBL") {
    ...
    joinGenes <- dplyr::left_join(modelGeneId, y, by = "ENSEMBL") |>
    ...
    message(paste0("The following genes used in normalising library size are missing from the input: ", 
                   paste0(missingNormGenes$ENSEMBL, collapse = ", "),
    ...
    message(paste0("The following genes used in normalising library size, that are also weighted for IdentifiHR predictions, are missing from the input: ", 
                   paste0(missingNormWeightedGenes$ENSEMBL, collapse = ", "),
